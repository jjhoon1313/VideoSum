...
def get_flabel(frames, fnum, fps, seg_l):
    ...
    s_i = [int(seg_fn[0]) for seg_fn in frames]
    e_i = [s + fps * seg_l for s in s_i]
    e_i = map(round, e_i)
    e_i = map(int, e_i)

    label = np.zeros((int(fnum), 1))
...
if __name__ == '__main__':
    ...
    frames = [[123, 124, 125, 126, 127], [222, 223, 224, 225, 226], [333 , 334, 335, 336, 337]]
    label = get_flabel(frames, 2914, 30, 5)
    print(label)
    print(label[:200].ravel().astype(np.bool))